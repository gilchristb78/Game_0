...
public:	
	...
	UItem* SwapItem(class UItem* Item, int32 index);
	bool Swap(int32 from, int32 to);
	...
	int32 ConsumeItemClass(TSubclassOf<class UItem> ItemClass, const int32 Quantity);
...
private:
	...
	FItemAddResult tryAddItemMultipleStacks_Internal(class UItem* ExistingItem, class UItem* Item, float QuantityToAdd, float totalTryAdd, FText ErrorText);
